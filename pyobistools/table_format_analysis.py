    ...
    return (
        html.Div(
            [
                ...
                html.H6(
                    "La présence des champs attendus pour un type d'analyse et de fichier est analysé ici-bas:"
                ),
                html.H6(
                    "The presence of the expected fields for a type of analysis and file is analyzed below:"
                ),
                dash_table.DataTable(
                    data=colonne_analyse.to_dict("records"),
                    id={"type": "colonne_analyse"},
                    columns=[
                        {
                            "name": ["Validation de format / Format validation", i],
                            "id": i,
                        }
                        for i in colonne_analyse.columns
                    ],
                    sort_action="native",
                ...
                html.P(""),
                html.H6(
                    "Les colonnes suivantes font parties du jeu de données et ne sont pas prise en compte dans le tableau précédent:"
                ),
                html.H6(
                    "The following columns are part of the dataset and are not taken into account in the previous table:"
                ),
                ...
                html.P(""),
                html.Div(
                    children=[liste_affichage[i] for i in range(len(liste_affichage))]
                ),
            ]
        ),
    )
