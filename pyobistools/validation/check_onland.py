...
def check_onland(data, land=None, report=False, buffer=0, offline=False):
    ...
    gdf = gpd.GeoDataFrame(data, 
                    geometry=gpd.points_from_xy(data.decimalLongitude, 
    ...
    if (offline and land is None):
        land_shp_fname=shpreader.natural_earth(resolution='10m', category='physical', name='land')
    ...
    if (offline):
        ...
        shoredistance = xy.lookup(data[['decimalLongitude', 'decimalLatitude']].to_numpy(), shoredistance=True, grids=True, areas = True, asdataframe = True)
        # gdf on_land needs to be a boolean series.
        gdf['on_land'] = shoredistance['shoredistance'] < 0
    # Are we returning a report or the offending data?
    if (report):
        if (len(i) > 0):
            return(gdf[gdf['on_land']]) # TODO: return a report format describing which rows are land-bound
        else:
            return() # return empty dataframe? empty row?
