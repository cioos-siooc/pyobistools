...
def validate_basisofrecord(data):
    ...
    tableau_basisofrecord = pd.DataFrame(data=data["basisofrecord"].value_counts(dropna=False))
    tableau_basisofrecord.reset_index(inplace=True)

    tableau_basisofrecord.rename(columns={"index":"Valeur / Value","basisofrecord": "Nombre d'observations / Number of observations"}, inplace=True)
    tableau_basisofrecord = tableau_basisofrecord.reindex(columns = ['Valeur / Value','DWC standard',"Nombre d'observations / Number of observations"])
    tableau_basisofrecord['DWC standard'] = tableau_basisofrecord["Valeur / Value"].isin(valeur_exemples)
    tableau_basisofrecord['DWC standard'].replace(False,"Absence", inplace=True)
    tableau_basisofrecord['DWC standard'].replace(True,"Présent / Present", inplace=True)

    liste_basisofrecord = ["Les valeurs trouvées dans la colonne 'basisofrecord' sont présentées dans le tableau et leur présence ou non dans la documentation est analysée.",
    ...
    return html.Div([
        ...
        data=tableau_basisofrecord.to_dict('records'),id={'type': 'tableau_basisofrecord'},
        columns=[{'name': ["Validation de données / Data validation: 'basisofrecord'", i], 'id': i} for i in tableau_basisofrecord.columns],
