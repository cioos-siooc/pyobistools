...
def validate_decimal_coordinates(data):
    ...
    latitudes_non_valides = data1.loc[
        (data1[colonne1].gt(90)) | (data1[colonne1].lt(-90))
    ]
    longitudes_non_valides = data1.loc[
        (data1[colonne2].gt(180)) | (data1[colonne2].lt(-180))
    ]
    ...
    coordonees_non_valides = coordonees_non_valides.iloc[
        :, [0, latitude_colonne, longitude_colonne]
    ]
    ...
    coordonnees_valides = data1[
        data1[colonne1].between(-90, 90) & data1[colonne2].between(-180, 180)
    ]
        ...
        return html.Div(
            [
                dash_table.DataTable(
                    data=coordonees_non_valides.to_dict("records"),
                    id={"type": "coordonees_non_valides"},
                    columns=[
                        {
                            "name": [
                                "Coordonnées non valides / Invalid coordinates",
                                i,
                            ],
                            "id": i,
                        }
                        for i in coordonees_non_valides.columns
                    ],
                    sort_action="native",
    ...
    if len(coordonees_non_valides) != 0:
        liste_affichage1.append(
            afficher_tableau_coordo_non_valides(coordonees_non_valides)
        )
    ...
    centre_long = 0.5 * (
        max(np.asarray(np.float64(coordonnees_valides[colonne2]).tolist()))
        + min(np.asarray(np.float64(coordonnees_valides[colonne2]).tolist()))
    )
    centre_lat = 0.5 * (
        max(np.asarray(np.float64(coordonnees_valides[colonne1]).tolist()))
        + min(np.asarray(np.float64(coordonnees_valides[colonne1]).tolist()))
    )
    height = max(np.asarray(np.float64(coordonnees_valides[colonne1]).tolist())) - min(
        np.asarray(np.float64(coordonnees_valides[colonne1]).tolist())
    )
    width = max(np.asarray(np.float64(coordonnees_valides[colonne2]).tolist())) - min(
        np.asarray(np.float64(coordonnees_valides[colonne2]).tolist())
    )
    area = height * width

    zoom = np.interp(
        x=area, xp=[0.0005, 0.02, 0.05, 30, 350, 3500], fp=[12, 9.5, 6, 4, 2, 1]
    )
    ...
    fig = go.Figure(
        go.Scattermapbox(
            lon=np.asarray(np.float64(coordonnees_valides[colonne2]).tolist()),
            lat=np.asarray(np.float64(coordonnees_valides[colonne1]).tolist()),
            mode="markers",
    ...
    liste_decimal_coordinates = [
        "Les paires de coordonnées des colonnes 'decimalLatitude' et 'decimalLongitude' sont représentées sur une carte géographique.",
    ...
    return html.Div(
        [
            ...
            html.Div(
                children=[liste_affichage1[i] for i in range(len(liste_affichage1))]
            ),
            ...
            html.Details(
                [
                    html.Summary(
                        "Info sur/on 'decimalLatitude' et/and 'decimalLongitude'"
                    ),
