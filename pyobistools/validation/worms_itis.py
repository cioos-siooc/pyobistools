...
from pyobistools.validation.sub_routines.suffix_function import (
    function_suffix_removal,
    function_add_suffix,
)
from pyobistools.validation.sub_routines.analysis_function import (
    names_analyse,
    names_ids_analyse,
    names_taxons_ids_analyse,
)
from pyobistools.validation.sub_routines.tables_function import (
    names_table,
    names_ids_tables,
    names_taxons_ids_tables,
)
...
def validation_Worms_Itis_fonction(data, value):
    ...
    data_valid_scientific_name = data_valid_scientific_name.drop_duplicates(
        subset=["scientificname"]
    )
    data_valid_scientific_name.reset_index(drop=True)
    ...
    ) = function_suffix_removal(data_valid_scientific_name)
        ...
        async with httpx.AsyncClient(timeout=timeout) as client:
            ...
            if response.status_code == 200:
                for key in list_of_list:
                    ...
                    data_valid_scientific_name.loc[
                    ...
                    data_valid_scientific_name.loc[
                        ...
                        == list_of_list[key],
                        "Valid_TaxonID",
                    ] = response2[0]["valid_AphiaID"]
                    data_valid_scientific_name.loc[
                        data_valid_scientific_name["scientificname"]
                        == list_of_list[key],
                        "Valid_Name",
                    ] = response2[0]["valid_name"]
                    data_valid_scientific_name.loc[
                        data_valid_scientific_name["scientificname"]
                        == list_of_list[key],
                        "LSID",
                    ...
                    print(
                        f"{index} : {response.status_code}: Worms {list_of_list[key]} "
                    )
            ...
            if response.status_code == 204:
                try:
                    ...
                    response3 = await client.get(
                        f"https://www.itis.gov/ITISWebService/jsonservice/searchByScientificName?srchKey={nom}"
                    )
                    ...
                    if response4["scientificNames"] != [None]:
                        for key in list_of_list:
                            ...
                            data_valid_scientific_name.loc[
                                ...
                                == list_of_list[key],
                                "Valid_Name",
                            ] = response4["scientificNames"][0]["combinedName"]
                            data_valid_scientific_name.loc[
                                data_valid_scientific_name["scientificname"]
                                == list_of_list[key],
                                "LSID",
                            ] = (
                                "urn:lsid:itis.gov:itis_tsn:"
                                + response4["scientificNames"][0]["tsn"]
                            )
                            print(
    ...
    if value == "names":
        return names_table(data_valid_scientific_name)
    if value == "names_ids":
        return names_ids_tables(data_valid_scientific_name, data_cross_validation)
    if value == "names_taxons_ids":
        return names_taxons_ids_tables(
            data_valid_scientific_name, data_cross_validation
        )
