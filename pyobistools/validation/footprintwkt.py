...
def validate_foorprintwkt(data):
    ...
    # Polygons list
    liste_large_polygones = list_features[
        list_features["footprintwkt"].str.contains("POLYGON")
    ]
    ...
    # Linestrings list
    liste_large_linestrings = list_features[
        list_features["footprintwkt"].str.contains("LINESTRING")
    ]
    ...
    # Points list
    liste_large_points = list_features[
        list_features["footprintwkt"].str.contains("POINT")
    ]
    ...
    def function_show_points(noms, liste_de_WKT):
        ...
        fig.update_layout(
            mapbox={
                ...
                "zoom": 4,
                "layers": [
                    {"source": layer, "below": "traces", "color": "royalblue"}
                    for layer in liste_de_WKT
                ],
            },
            margin={"l": 0, "r": 0, "b": 0, "t": 0},
    ...
    if len(liste_large_polygones[0]["features"]) != 0:
        list_maps.append(
            function_show_polygons(
                "'Polygon' et/and 'MultiPolygon'", liste_large_polygones
            )
        )
    ...
    if len(liste_large_linestrings[0]["features"]) != 0:
        list_maps.append(
            function_show_lines(
                "'LineString' et/and 'MultiLineString'", liste_large_linestrings
            )
        )

    if len(liste_large_points[0]["features"]) != 0:
        list_maps.append(
            function_show_points("'Point' et/and 'MultiPoint'", liste_large_points)
        )

    list_footprintwkt = [
        "Les différents objets du champ 'footprintWKT' sont représentées sur des cartes géographiques.",
