...
def validate_datasetid(data):
    ...
    tableau_datasetid.rename(
        columns={
            "index": "Valeurs / Values",
            "datasetid": "Nombre d'observations / Number of observations",
        },
        inplace=True,
    )

    liste_datasetid = [
        "Les valeurs trouvées dans la colonne 'datasetID' sont présentées dans le tableau.",
    ...
    return html.Div(
        [
            ...
            dash_table.DataTable(
                data=tableau_datasetid.to_dict("records"),
                id={"type": "tableau_datasetid"},
                columns=[
                    {
                        "name": [
                            "Validation de données / Data validation: 'datasetID'",
                            i,
                        ],
                        "id": i,
                    }
                    for i in tableau_datasetid.columns
                ],
                sort_action="native",
