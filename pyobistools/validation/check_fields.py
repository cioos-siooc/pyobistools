...
def check_fields(data, level = 'error', analysis_type = 'occurrence_core', accepted_name_usage_id_check = False):
    ...
    if level == "warning":
        ...
        analysis_field = dataframe_column_key.loc[dataframe_column_key['Required or recommended'] == 'Recommended field']

    # # SECTION FOR FORMAT ANALYSIS
    if analysis_field.empty == True:
    ...
    if accepted_name_usage_id_check:
        if 'acceptednameusageid' in dataset_column_names:
            ...
            index_of_filtered_data = data[(data['scientificnameid'].isna()) &  (data['acceptednameusageid'].str.len() > 6) ].index
            ...
            field_analysis3 = pd.concat([analysis_field, field_analysis3]).drop_duplicates( keep=False)
