...
def AfficheOccurrenceParValeur(data, nomcolonne):
    ...
    tableau_valeur.rename(
        columns={
            "index": "Valeurs / Values",
            nomcolonne: "Nombre d'observations / Number of observations",
        },
        inplace=True,
    )

    return dash_table.DataTable(
        data=tableau_valeur.to_dict("records"),
        id={"type": "tableau_valeur"},
        columns=[
            {
                "name": [
                    "Validation de données / Data validation: '" + nomcolonne + "'",
                    i,
                ],
                "id": i,
            }
            for i in tableau_valeur.columns
        ],
        sort_action="native",
