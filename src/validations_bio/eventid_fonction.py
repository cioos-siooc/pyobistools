...
def eventid_fonction(data):
    data = data.rename(columns=str.lower)
    
    tableau_eventid = pd.DataFrame(data=data["eventid"].value_counts(dropna=False))
    tableau_eventid.reset_index(inplace=True)

    tableau_eventid.rename(columns={"index":"Valeurs / Values","eventid": "Nombre d'observations / Number of observations"}, inplace=True)

    liste_eventid = ["Les valeurs trouvées dans la colonne 'eventID' sont présentées dans le tableau.",
    ...
    return html.Div([
        ...
        data=tableau_eventid.to_dict('records'),id={'type': 'tableau_eventid'},
        columns=[{'name': ["Validation de données / Data validation: 'eventid'", i], 'id': i} for i in tableau_eventid.columns],
